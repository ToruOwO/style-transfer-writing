...
based on char-rnn Written by Andrej Karpathy (@karpathy)

"""
import numpy as np

# data IO
data = open('data/oscar_wilde_lines.txt').read().split()
chars = list(set(data))
...
seq_length = 50 # number of steps to unroll RNN for
...
def loss_func(inputs, targets, hprev):
    ...
    :param inputs: List[int]
    :param targets: List[int]
    :param hprev: Hx1 array of initial hidden state

    :return: the loss, gradients on model params and last hidden state
    """
    xs, hs, ys, ps = {}, {}, {}, {}
    hs[-1] = np.copy(hprev)
    loss = 0
